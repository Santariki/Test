...
class Particle:
	...
	def updatepos(self,graph):
		...
		for i in graph:
			...
			if(self.velocity[j]):
				...
				for k in temp:
					n.append(graph.nodes[k]['pos'])
					#print(n)
				if(len(n)!=len(set(n))):
					p=Counter(n).most_common(1)[0][0]
					graph.nodes[i]['pos']=p
				else:
					if(graph.nodes[i]['pos'] in n):
						pass
					else:
	...
	def updatevelocity(self,graph,k):
		...
		j=0
		for i in graph:
			v1.append(int((self.pbest[j]==graph.nodes[i]['pos']) and '0' or '1'))
			v2.append(int((self.gbest[j]==graph.nodes[i]['pos']) and '0' or '1'))
	...
	def fitness(self,graph):
		...
		for j in graph:	
			for i in graph:
				...
				gama=int(graph.nodes[j]['pos']==graph.nodes[i]['pos'])
	...
	def rearrange(self,graph):
		...
		for i in single:
			...
			new_pos.append(num)
			list(node).remove(num)
			if(f is True):
				list(node).append(i)
		for i in graph:
			t=graph.nodes[i]['pos']
			d=single.index(t)
			graph.nodes[i]['pos']=new_pos[d]
	...
	def optimize(self):
		...
		j=0		
		for i in self.G:
			self.G.nodes[i]['pos']=self.gbest[j]
			j+=1
		print("\n\n**********************************************************")	
		...
		print("\nGraph : ",self.G.nodes)
		community={}
		gbestSet=list(set(self.gbest))

		for i,k in zip(range(len(self.G.nodes)),self.G.nodes):
			for j in gbestSet:
				if(self.gbest[i]==j):
					if j in community.keys():
						community[j].append(k)
					else:
						community.setdefault(j,[])
						community[j].append(k)

				else:
					pass
		for i in community:
			print(community.get(i))
		# nx.draw(self.G,node_color=[self.G.nodes[i]['pos'] for i in self.G])
		# plt.show()
